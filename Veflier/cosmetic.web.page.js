...
async function fetchReviews() {
    ...
    reviewContainer.innerHTML = reviews
      .map(
        (r) => 
            `<div>
                    <img src="${r.profile_picture}" alt="${r.email}" style="width: 50px; height: 50px; border-radius: 50%;" />
                    <strong>${r.email}</strong> <small>${new Date(r.date).toLocaleDateString()}</small>
                    <p>${r.review}</p>
                </div>`
      )
      .join("");
  ...
  async function submitReview(event) {
    ...
    await fetch("http://localhost:3000/reviews", {
      ...
      body: JSON.stringify({ profile_picture, email, review }),
    });
